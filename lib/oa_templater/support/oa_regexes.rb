module OaTemplater
  ...
  R_HEADER_TYPES = /( (?:(?:..)?(?:＜|（|［|【)(?:本願)*(?:請\p{Z}*求\p{Z}*項|引用文献|理\p{Z}*由|備\p{Z}*考|実\p{Z}*施\p{Z}*例)(?:\p{Z}*.*\p{N}，*\p{Z}*)+(?:(?:について)*＞|）|］|】))
                     |  (?:^\p{Z}*(?:・.*)\p{N}(?:について)*\p{Z}*$)
  ...
  R_CITATIONS_START = /(引　用　文　献　等　一　覧|引用文献(等)?一覧|引用文献等|引\p{Z}*用\p{Z}*文\p{Z}*献|引用刊行(物)?).?(?:<\/CENTER>)*\s+\p{Z}*\p{N}+?(?:\.|．|：)*\p{Z}*/m
